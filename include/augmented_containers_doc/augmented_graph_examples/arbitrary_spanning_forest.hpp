...
std::vector<std::vector<std::string>> AUGMENTED_GRAPH_EXAMPLE_PREFIXING(to_graphs_string)()
{
    ...
    for (std::vector<dot::graph_t> &graphs : to_graphs(
             ...
             to_graphs_parameters_t{
                ...
                .edge_to_string_converter = [](auto parameters) -> html_label_t { return html_label_t{text_t{{{std::u8string(reinterpret_cast<char8_t const *>((std::ostringstream() << "[" << std::ranges::distance(AUGMENTED_GRAPH_EXAMPLE_PREFIXING(augmented_graph).begin_edge(), parameters.datum) << "]").str().data()))}}}}; },
                .part_data_to_string_converter_per_part = std::make_tuple(std::nullopt),
