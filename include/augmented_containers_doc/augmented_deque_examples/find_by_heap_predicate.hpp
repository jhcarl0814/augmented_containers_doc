...
//lookup
void AUGMENTED_DEQUE_EXAMPLE_PREFIXING(find_by_heap_predicate)(std::size_t I, std::string value)
{
    if (I == 0)
    {
        int value_int;
        ...
        AUGMENTED_DEQUE_EXAMPLE_PREFIXING(augmented_deque).find_by_heap_predicate<0>(std::back_inserter(std::get<0>(AUGMENTED_DEQUE_EXAMPLE_PREFIXING(augmented_deque_iterators_projected_storage_per_sequence))), [&](decltype(AUGMENTED_DEQUE_EXAMPLE_PREFIXING(augmented_deque))::sequence_t<0>::accumulated_storage_t const &element_or_max_element_in_tree) { return static_cast<int>(element_or_max_element_in_tree) >= value_int; });
    }
}
...
std::vector<std::string> AUGMENTED_DEQUE_EXAMPLE_PREFIXING(to_graphs_string)()
{
    ...
    for (dot::graph_t &graph : to_graphs(
             ...
             to_graphs_parameters_t{
                ...
                .projected_and_accumulated_storage_to_string_converter_per_sequence = std::make_tuple(std::make_pair([](auto parameters) -> html_label_t { return html_label_t{text_t{{{std::u8string(reinterpret_cast<char8_t const *>((std::ostringstream() << parameters.datum).str().data()))}}}}; }, [](auto parameters) -> html_label_t { return html_label_t{text_t{{{std::u8string(reinterpret_cast<char8_t const *>((std::ostringstream() << parameters.datum).str().data()))}}}}; })),
