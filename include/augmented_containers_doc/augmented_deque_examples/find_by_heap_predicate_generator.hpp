...
{
    return std::ranges::prev(AUGMENTED_DEQUE_EXAMPLE_PREFIXING(augmented_deque_iterators_element)[iterator_index]).is_end()
        ? static_cast<std::int32_t>(AUGMENTED_DEQUE_EXAMPLE_PREFIXING(augmented_deque_prev_element))
        : static_cast<std::int32_t>(std::ranges::prev(AUGMENTED_DEQUE_EXAMPLE_PREFIXING(augmented_deque_iterators_element)[iterator_index])->first);
}
std::int32_t AUGMENTED_DEQUE_EXAMPLE_PREFIXING(next_lower_endpoint)(std::size_t iterator_index)
{
    return std::ranges::next(AUGMENTED_DEQUE_EXAMPLE_PREFIXING(augmented_deque_iterators_element)[iterator_index]).is_end()
        ? static_cast<std::int32_t>(AUGMENTED_DEQUE_EXAMPLE_PREFIXING(augmented_deque_next_element))
        : static_cast<std::int32_t>(std::ranges::next(AUGMENTED_DEQUE_EXAMPLE_PREFIXING(augmented_deque_iterators_element)[iterator_index])->first);
}
