...
void AUGMENTED_DEQUE_EXAMPLE_PREFIXING(find_by_monotonic_predicate)(std::size_t I, std::string value)
{
    if (I == 0)
    {
        ...
        std::get<0>(AUGMENTED_DEQUE_EXAMPLE_PREFIXING(augmented_deque_iterators_projected_storage_per_sequence))
            [0] = AUGMENTED_DEQUE_EXAMPLE_PREFIXING(augmented_deque).find_by_monotonic_predicate<0>([&](decltype(AUGMENTED_DEQUE_EXAMPLE_PREFIXING(augmented_deque))::sequence_t<0>::accumulated_storage_t const &accumulated_string_size) { return static_cast<int>(accumulated_string_size) > value_int; });
    }
}
