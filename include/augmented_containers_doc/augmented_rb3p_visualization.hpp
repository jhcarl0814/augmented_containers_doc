...
namespace augmented_containers
{
    ...
    #define AUGMENTED_CONTAINERS_VISUALIZATION_COMMON
            ...
            {
                std::ranges::for_each(reinterpret_cast<std::byte const(&)[sizeof(T)]>(print_pointer.p), [&](std::byte const &b) { os << "0x" << std::hex << std::setw(2) << std::setfill('0') << (int) b << std::dec << std::setw(0) << std::setfill(' '); });
            ...
            {
                enum class accumulated_storage_node_type_e {
                    uninitialized,
                    tree_node,
                    read_range
                };
    ...
    std::vector<dot::graph_t> to_graphs(std::vector<augmented_sequence_t<element_t, allocator_element_t, accumulator_t, std::integral_constant<augmented_sequence_physical_representation_e, augmented_sequence_physical_representation_e::rb3p>, size_management_t>> const &augmented_sequence_rb3p_s, detail::visualization::augmented_sequence_rb3p::to_graphs_parameters_t<to_graphs_parameters_t_parameters_t...> to_graphs_parameters)
    {
        ...
        for (container_t const &augmented_sequence_rb3p : augmented_sequence_rb3p_s)
        {
            ...
            node_statement_t tree_node_end_members_render_from_left_to_right_node{
                {u8"tree_node_end_" + object_pointer_to_string(p_node_end) + u8"_members_render_from_left_to_right"},
                {{
            ...
            tree_node_end_subgraph.statements.push_back(node_statement_t{
                {u8"tree_node_end_" + object_pointer_to_string(p_node_end) + u8"_child_right"},
                {{
                    ...
                    {u8"label", html_label_t{fonttable_t{table_t{
                                    ...
                                    .CELLSPACING = 0,
                                    .COLOR = u8"#888888",
                                    .rows = [&]() { return decltype(table_t::rows){
                                                        row_t{{
                                                            cell_t{
            ...
            tree_node_end_subgraph.statements.push_back(node_statement_t{
                {u8"tree_node_end_" + object_pointer_to_string(p_node_end) + u8"_parent"},
                {{
            ...
            tree_node_end_subgraph.statements.push_back(node_statement_t{
                {u8"tree_node_end_" + object_pointer_to_string(p_node_end) + u8"_child_left"},
                {{
            ...
            std::function<void(typename container_t::node_t *, std::optional<std::reference_wrapper<std::vector<node_statement_t>>>)> tree_node_recursive = [&](typename container_t::node_t *p_tree_node, std::optional<std::reference_wrapper<std::vector<node_statement_t>>> tree_nodes_from_black_parent) {
                ...
                tree_nodes.push_back(node_statement_t{
                    {u8"tree_node_" + object_pointer_to_string(p_tree_node)},
                    {{
            ...
            if constexpr (!std::is_same_v<typename container_t::accumulated_storage_t, void>)
            {
                ...
                {
                    read_range_accumulated_storages.statements.push_back(node_statement_t{
                        {u8"read_range_" + object_pointer_to_string(p_node_end)},
                        {{
                            ...
                            {u8"label", to_graphs_parameters.accumulated_storage_to_string_converter(to_string_converter_parameters_t{
                                            ...
                                            .datum = augmented_sequence_rb3p.read_range(to_graphs_parameters.iterators[2 * container_index + 0], to_graphs_parameters.iterators[2 * container_index + 1]),
                                            .converter_generated_statements_back_inserter = std::back_inserter(converter_generated_statements),
        ...
        std::ranges::for_each(to_graphs_parameters.iterators, [&, index = 0](auto iterator) mutable {
            if (std::ranges::none_of(augmented_sequence_rb3p_s, [&](container_t const &augmented_sequence_rb3p) { return iterator.current_node == augmented_sequence_rb3p.end(); }))
            {
                iterator_nodes.push_back(node_statement_t{
                    {u8"it[" + index_to_string(index) + u8"]"},
                    {{
                        {u8"id", u8"it[" + index_to_string(index) + u8"]"},
                        {u8"label", u8"it[" + index_to_string(index) + u8"]" + [&]() -> std::u8string {if constexpr(container_t::iterator_t::support_random_access)return u8"\\l.index():"+index_to_string(iterator.index());else return u8""; }()},
                    }}
                });
        ...
        if constexpr (!std::is_same_v<decltype(to_graphs_parameters.iterators_output), std::nullopt_t>)
        {
            std::ranges::for_each(to_graphs_parameters.iterators_output, [&, index = 0](auto iterator) mutable {
                ...
                {
                    iterator_nodes.push_back(node_statement_t{
                        {u8"jt[" + index_to_string(index) + u8"]"}, {{
