...
std::uniform_int_distribution<int> AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(distrib_element)('a', 'z');
std::uniform_int_distribution<std::size_t> AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(distrib_size)(0, 5);

struct AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(accumulating_binary_functor_string_length)
{
    using accumulated_storage_t = std::size_t;
    std::size_t identity_element() const { return 0; }
    std::size_t operator()(std::size_t &accumulated_size, std::string const &s) const
    {
        return accumulated_size + s.size();
    }
    std::size_t operator()(std::size_t &accumulated_size, std::size_t &size) const
    {
        return accumulated_size + size;
    }
};
std::vector<
    augmented_containers::augmented_sequence_t<
        ...
        augmented_containers::augmented_sequence_helpers::accumulator_wrapping_accumulating_binary_functor_t<AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(accumulating_binary_functor_string_length)>,
...
{
    std::string result;
    std::ranges::generate_n(std::back_inserter(result), AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(distrib_size)(AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(gen)), std::bind(std::ref(AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(distrib_element)), std::ref(AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(gen))));
    AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(augmented_sequence_iterators)
    [iterator_index] = AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(augmented_sequence_s)[AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(augmented_sequence_iterators_container_index)[iterator_index]].insert(AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(augmented_sequence_iterators)[iterator_index], result);
}
void AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(iterator_erase)(std::size_t iterator_index)
{
    if (AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(augmented_sequence_iterators)[iterator_index] != AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(augmented_sequence_s)[AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(augmented_sequence_iterators_container_index)[iterator_index]].end())
    {
        ...
        std::ranges::transform(AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(augmented_sequence_iterators_output), std::back_inserter(iterators_output_will_be_invalidated), [&](decltype(AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(augmented_sequence_s))::value_type::iterator_t iterator) { return iterator == AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(augmented_sequence_iterators)[iterator_index]; });
#else
        ...
        std::vector<bool> iterators_output_will_be_invalidated = AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(augmented_sequence_iterators_output) | std::views::transform([&](decltype(AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(augmented_sequence_s))::value_type::iterator_t iterator) { return iterator == AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(augmented_sequence_iterators)[iterator_index]; }) |
            std::ranges::to<std::vector>();
#endif
        AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(augmented_sequence_iterators)
        ...
        std::ranges::for_each(iterators_output_will_be_invalidated, [&, index = 0](bool iterator_output_will_be_invalidated) mutable {if(iterator_output_will_be_invalidated)AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(augmented_sequence_iterators_output)[index]=AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(augmented_sequence_iterators)[iterator_index];++index; });
    }
}
void AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(iterator_split)(std::size_t iterator_index)
{
    ...
    if (AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(augmented_sequence_iterators_container_index)[iterator_index] == 0)
    {
        ...
        if (AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(augmented_sequence_iterators_output_container_index) == 1)
            AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(augmented_sequence_iterators_output)
        [0] = AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(augmented_sequence_s)[1].end();
    }
    else if (AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(augmented_sequence_iterators_container_index)[iterator_index] == 1)
    {
        ...
        if (AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(augmented_sequence_iterators_output_container_index) == 0)
            AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(augmented_sequence_iterators_output)
        [0] = AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(augmented_sequence_s)[0].end();
    }
...
void AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(find_by_monotonic_predicate)(std::size_t container_index, std::string value)
{
    int value_int;
    std::istringstream(value) >> value_int;
    AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(augmented_sequence_iterators_output)
    [0] = AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(augmented_sequence_s)[container_index].find_by_monotonic_predicate([&](decltype(AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(augmented_sequence_s))::value_type::accumulated_storage_t const &accumulated_string_size) { return static_cast<int>(accumulated_string_size) > value_int; });
...
std::vector<std::string> AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(to_graphs_string)()
{
    ...
    for (dot::graph_t &graph : to_graphs(
             ...
             to_graphs_parameters_t{
                ...
                .accumulated_storage_to_string_converter = [](auto parameters) -> html_label_t { return html_label_t{text_t{{{std::u8string(reinterpret_cast<char8_t const *>((std::ostringstream() << parameters.datum).str().data()))}}}}; },
