...
AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(accumulating_group_by_element_binary_functor_t)::accumulated_storage_t AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(accumulating_group_by_element_binary_functor_t)::operator()(accumulated_storage_t & lhs, int const &rhs) const
{
    ...
    {
        it = result.key_and_node_ends.insert //
             ({
                  group_by_key,
                  AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(group_by_element_node_end_t){
                                                                                    .front = tagged_ptr_bit0_setted(&iterator_rhs.to_pointer_accumulated_storage()->node),
                                                                                    .back = tagged_ptr_bit0_setted(&iterator_rhs.to_pointer_accumulated_storage()->node),
                                                                                    }
        })
                 .first;
             ...
             to_graphs_parameters_t{
                ...
                .accumulated_storage_to_string_converter = [](auto parameters) -> html_label_t {
                    ...
                    std::u8string color_primary = std::map<std::size_t, std::u8string>{
                        ...
                        {1, u8"#00FF00"},
                        {2, u8"#0000FF"}
                    }.at(AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(calculate_group_by_s_key)(*iterator_from));
                    ...
                    if (parameters.datum.node.prev != nullptr)
                    {
                        ...
                        {
                            if (std::ranges::none_of(parameters.datum.key_and_node_ends, [&](std::pair<std::size_t const, AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(group_by_element_node_end_t)> const &key_and_node_end) { return parameters.datum.node.prev == reinterpret_cast<AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(group_by_element_node_t) const *>(tagged_ptr_bit0_setted(&key_and_node_end.second)); }))
                            {
                    ...
                    if (parameters.datum.node.next != nullptr)
                    {
                        ...
                        {
                            if (std::ranges::none_of(parameters.datum.key_and_node_ends, [&](std::pair<std::size_t const, AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(group_by_element_node_end_t)> const &key_and_node_end) { return parameters.datum.node.next == reinterpret_cast<AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(group_by_element_node_t) const *>(tagged_ptr_bit0_setted(&key_and_node_end.second)); }))
                            {
                                decltype(AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(augmented_sequence_s))::value_type::iterator_t iterator_to = iterator_root;
                                parameters.converter_generated_statements_back_inserter++ = edge_statement_t{
                                    {{
                    ...
                    text_t text_C1001_node_prev{{textitem_t{std::make_pair(font_t{.COLOR = color_primary}, text_t{{textitem_t{std::ranges::any_of(parameters.datum.key_and_node_ends, [&](std::pair<std::size_t const, AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(group_by_element_node_end_t)> const &key_and_node_end) { return parameters.datum.node.prev == reinterpret_cast<AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(group_by_element_node_t) const *>(tagged_ptr_bit0_setted(&key_and_node_end.second)); }) ? u8"↑" : u8" "}}})}}};
                    text_t text_C1001_node_next{{textitem_t{std::make_pair(font_t{.COLOR = color_primary}, text_t{{textitem_t{std::ranges::any_of(parameters.datum.key_and_node_ends, [&](std::pair<std::size_t const, AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(group_by_element_node_end_t)> const &key_and_node_end) { return parameters.datum.node.next == reinterpret_cast<AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(group_by_element_node_t) const *>(tagged_ptr_bit0_setted(&key_and_node_end.second)); }) ? u8"↑" : u8" "}}})}}};
