...
void AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(find_by_heap_predicate_generator)(std::size_t container_index, std::string value)
{
    ...
    if(auto threshold_and_denominator = string_to_threshold_and_denominator(value); threshold_and_denominator)
    {
        ...
        AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(augmented_sequence_iterators_output) = AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(augmented_sequence_s)[container_index].find_by_heap_predicate_generator([&](decltype(AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(augmented_sequence_s))::value_type::accumulated_storage_t const &element_or_max_element_in_tree)
                                                                                        { return static_cast<int>(element_or_max_element_in_tree) >= threshold_and_denominator->first; }) |
            std::views::take_while([&](decltype(AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(augmented_sequence_s))::value_type::iterator_t jt)
                { return *jt % threshold_and_denominator->second != 0; }) |
            std::ranges::to<std::vector>();
#endif
    }
}
