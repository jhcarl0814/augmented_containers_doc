...
std::size_t AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(iterators_count)() { return AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(augmented_sequence_iterators).size(); }
...
#else
        std::vector<bool> iterators_will_be_invalidated = AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(augmented_sequence_iterators) | std::views::transform([&](decltype(augmented_sequence_s)::value_type::iterator_t iterator) { return iterator == AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(augmented_sequence_iterators)[iterator_index]; }) |
...
std::vector<std::string> AUGMENTED_SEQUENCE_EXAMPLE_PREFIXING(to_graphs_string)()
{
    ...
    for (dot::graph_t &graph : to_graphs(
             ...
             to_graphs_parameters_t{
                ...
                .accumulated_storage_to_string_converter = std::nullopt,
