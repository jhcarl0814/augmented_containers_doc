    ...
    #pragma clang diagnostic pop
                ...
                if constexpr (I == 0)
                {
                    sequence_members_render_from_left_to_right_node.attributes.back().push_back(attr_t{
                        u8"label",
                        std::u8string(reinterpret_cast<char8_t const *>((std::ostringstream() << "augmented_deque.sequence<" << I << ">()"
                                                                         //                                                                                              << R"( { )" //<< R"(\l)"
                ...
                if constexpr (generating_list_node_front_and_back)
                    sequence_members.statements.push_back(node_statement_t{
                        {u8"list_node_front"},
                        {{
                ...
                {
                    sequence_members_digit_end_accumulated_storage.statements.push_back(node_statement_t{
                        {u8"digit_node_end.accumulated_storage"},
                        {{
                ...
                subgraph_t digit_nodes_same_rank{
                    .statements = {
                                   attr_t{u8"rank", u8"same"},
                                   }
                };
                ...
                subgraph_t list_nodes_same_rank{
                    .statements = {
                                   attr_t{u8"rank", u8"same"},
                                   }
                };
                subgraph_t stride1_list_nodes_same_rank{
                    .statements = {
                                   attr_t{u8"rank", u8"same"},
                                   }
                };
                ...
                for (auto p_digit_node_navigator = sequence_t::navigator_t::untagged_next_or_tagged_end(sequence.digit_node_end); p_digit_node_navigator != sequence.digit_node_end; p_digit_node_navigator = p_digit_node_navigator->next)
                {
                    ...
                    digit_nodes.push_back(node_statement_t{
                        {u8"digit_node_" + object_pointer_to_string(p_digit_node)},
                        {{
                ...
                if constexpr (I == 0)
                {
                    for (auto p_list_node_navigator = sequence_t::navigator_t::untagged_next_or_tagged_end(sequence.list_node_end); p_list_node_navigator != sequence.list_node_end; p_list_node_navigator = p_list_node_navigator->next)
                    {
                        ...
                        list_nodes.push_back(node_statement_t{
                            {u8"list_node_" + object_pointer_to_string(p_list_node)},
                            {{
                ...
                std::ranges::for_each(to_graphs_parameters.iterators_element, [&, index = 0](auto iterator_element) mutable {
                    if ([&]() {if constexpr(I == 0)return iterator_element.current_list_node != sequence.list_node_end; else return iterator_element.current_list_node !=stride1_sequence.list_node_end; }())
                    {
                        iterator_element_nodes.push_back(node_statement_t{
                ...
                if constexpr (!std::is_same_v<decltype(to_graphs_parameters.iterators_projected_storage_per_sequence), std::nullopt_t>)
                {
                    std::ranges::for_each(std::get<I>(to_graphs_parameters.iterators_projected_storage_per_sequence), [&, index = 0](auto iterator_projected_storage) mutable {
                        ...
                        {
                            iterator_projected_storage_nodes.push_back(node_statement_t{
                                {u8"itp[" + index_to_string(index) + u8"]"}, {{
