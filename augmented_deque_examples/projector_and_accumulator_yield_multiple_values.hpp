...
struct AUGMENTED_DEQUE_EXAMPLE_PREFIXING(projector_min_n_element_t)
{
    ...
    projected_storage_t project_n_ary_functor(it_element_t it_chunk_begin, it_element_t it_chunk_end, std::size_t chunk_size) const
    {
        ...
        std::transform(it_chunk_begin, it_chunk_end, result.begin(), [](element_t const &element)
...
#endif
#ifdef __EMSCRIPTEN__
        std::stable_sort(result.begin(), result.begin() + chunk_size, [](element_t * const &lhs, element_t * const &rhs)
            {
    ...
    {
        assert(chunk_size != 0);
        new(storage) projected_storage_t(project_n_ary_functor(it_chunk_begin, it_chunk_end, chunk_size));
    }
...
template<typename element_t, std::size_t N, bool reverse = false>
struct AUGMENTED_DEQUE_EXAMPLE_PREFIXING(projector_and_accumulator_min_n_element_t): AUGMENTED_DEQUE_EXAMPLE_PREFIXING(projector_min_n_element_t)<element_t, N, reverse>, augmented_containers::augmented_deque_helpers::accumulator_wrapping_accumulating_binary_functor_t<AUGMENTED_DEQUE_EXAMPLE_PREFIXING(accumulating_min_n_element_binary_functor_t)<element_t, N, reverse>>
{};
                ...
                .projected_and_accumulated_storage_to_string_converters =
                    ...
                    std::make_pair(
                        [](auto parameters) -> html_label_t
                        {
                            ...
                            auto iterator_projected_storage = decltype(AUGMENTED_DEQUE_EXAMPLE_PREFIXING(augmented_deque))::sequence_t<1>::iterator_projected_storage_t::from_projected_storage_address(&const_cast<projected_storage_t &>(parameters.datum));
                            for(element_t * const &p : parameters.datum)
                            {
                                if(p != nullptr)
                                {
                                    auto iterator_element = decltype(AUGMENTED_DEQUE_EXAMPLE_PREFIXING(augmented_deque))::sequence_t<0>::iterator_element_t::from_element_address(p);
                        ...
                        [](auto parameters) -> html_label_t
                        {
                            ...
                            {
                                auto digit_node = decltype(AUGMENTED_DEQUE_EXAMPLE_PREFIXING(augmented_deque))::sequence_t<1>::digit_node_t::from_accumulated_storage_address(&const_cast<accumulated_storage_t &>(parameters.datum));
                                for(element_t * const &p : parameters.datum)
                                {
                                    if(p != nullptr)
                                    {
                            ...
                            {
                                auto tree_node = decltype(AUGMENTED_DEQUE_EXAMPLE_PREFIXING(augmented_deque))::sequence_t<1>::tree_node_t::from_accumulated_storage_address(&const_cast<accumulated_storage_t &>(parameters.datum));
                                for(element_t * const &p : parameters.datum)
                                {
                                    if(p != nullptr)
                                    {
                            ...
                            case accumulated_storage_node_type_t::read_range:
                            {
                                auto tree_node = decltype(AUGMENTED_DEQUE_EXAMPLE_PREFIXING(augmented_deque))::sequence_t<1>::tree_node_t::from_accumulated_storage_address(&const_cast<accumulated_storage_t &>(parameters.datum));
                                for(element_t * const &p : parameters.datum)
                                {
...
//                            auto iterator_projected_storage = decltype(AUGMENTED_DEQUE_EXAMPLE_PREFIXING(augmented_deque))::sequence_t<2>::iterator_projected_storage_t::from_projected_storage_address(&const_cast<projected_storage_t &>(parameters.datum));
//                            for(element_t * const &p : parameters.datum)
//                            {
//                                if(p != nullptr)
//                                {
...
//                            case accumulated_storage_node_type_t::digit_node:
//                            {
//                                auto digit_node = decltype(AUGMENTED_DEQUE_EXAMPLE_PREFIXING(augmented_deque))::sequence_t<2>::digit_node_t::from_accumulated_storage_address(&const_cast<accumulated_storage_t &>(parameters.datum));
//                                for(element_t * const &p : parameters.datum)
//                                {
//                                    if(p != nullptr)
//                                    {
...
//                            case accumulated_storage_node_type_t::tree_node:
//                            {
//                                auto tree_node = decltype(AUGMENTED_DEQUE_EXAMPLE_PREFIXING(augmented_deque))::sequence_t<2>::tree_node_t::from_accumulated_storage_address(&const_cast<accumulated_storage_t &>(parameters.datum));
//                                for(element_t * const &p : parameters.datum)
//                                {
//                                    if(p != nullptr)
//                                    {
