...
struct AUGMENTED_DEQUE_EXAMPLE_PREFIXING(projector_diffing_adjacent_element_t)
{
    ...
    void construct_projected_storage(projected_storage_t *storage, [[maybe_unused]] it_projected_storage_t it_this_list_node, [[maybe_unused]] it_element_t it_also_this_list_node) const
    {
        new(storage) projected_storage_t();
    }
...
struct AUGMENTED_DEQUE_EXAMPLE_PREFIXING(accumulator_diffing_adjacent_element_node_t)
{
    ...
    AUGMENTED_DEQUE_EXAMPLE_PREFIXING(accumulator_diffing_adjacent_element_node_t)() = default;
    AUGMENTED_DEQUE_EXAMPLE_PREFIXING(accumulator_diffing_adjacent_element_node_t)(AUGMENTED_DEQUE_EXAMPLE_PREFIXING(projector_diffing_adjacent_element_node_t) *front, AUGMENTED_DEQUE_EXAMPLE_PREFIXING(projector_diffing_adjacent_element_node_t) *back)
        : front(front),
          back(back)
    {
        using namespace augmented_containers::detail::tagged_ptr_ns;
        if(front != nullptr)
    ...
    AUGMENTED_DEQUE_EXAMPLE_PREFIXING(accumulator_diffing_adjacent_element_node_t)(AUGMENTED_DEQUE_EXAMPLE_PREFIXING(accumulator_diffing_adjacent_element_node_t) const &other)
...
struct AUGMENTED_DEQUE_EXAMPLE_PREFIXING(accumulating_diffing_adjacent_element_binary_functor_t)
{
    ...
    AUGMENTED_DEQUE_EXAMPLE_PREFIXING(accumulator_diffing_adjacent_element_node_t) monoidal() const { return {nullptr, nullptr}; }
    AUGMENTED_DEQUE_EXAMPLE_PREFIXING(accumulator_diffing_adjacent_element_node_t) operator()(AUGMENTED_DEQUE_EXAMPLE_PREFIXING(accumulator_diffing_adjacent_element_node_t) lhs, AUGMENTED_DEQUE_EXAMPLE_PREFIXING(projector_diffing_adjacent_element_node_t) const &rhs) const;
    AUGMENTED_DEQUE_EXAMPLE_PREFIXING(accumulator_diffing_adjacent_element_node_t) operator()(AUGMENTED_DEQUE_EXAMPLE_PREFIXING(accumulator_diffing_adjacent_element_node_t) lhs, AUGMENTED_DEQUE_EXAMPLE_PREFIXING(accumulator_diffing_adjacent_element_node_t) rhs) const;
};

struct AUGMENTED_DEQUE_EXAMPLE_PREFIXING(projector_and_accumulator_diffing_adjacent_element_t): AUGMENTED_DEQUE_EXAMPLE_PREFIXING(projector_diffing_adjacent_element_t), augmented_containers::augmented_deque_helpers::accumulator_wrapping_accumulating_binary_functor_t<AUGMENTED_DEQUE_EXAMPLE_PREFIXING(accumulating_diffing_adjacent_element_binary_functor_t)>
{};
...
bool AUGMENTED_DEQUE_EXAMPLE_PREFIXING(is_front_of_chunk)(decltype(AUGMENTED_DEQUE_EXAMPLE_PREFIXING(augmented_deque))::sequence_t<0>::iterator_element_t iterator_element)
{
    if(auto iterator_element_prev =
            ...
            std::ranges::prev
#endif
            (iterator_element); iterator_element_prev.is_end())
        return true;
    else
        return (*iterator_element_prev < 50) != (*iterator_element < 50);
}
AUGMENTED_DEQUE_EXAMPLE_PREFIXING(accumulator_diffing_adjacent_element_node_t) AUGMENTED_DEQUE_EXAMPLE_PREFIXING(accumulating_diffing_adjacent_element_binary_functor_t)::operator()(AUGMENTED_DEQUE_EXAMPLE_PREFIXING(accumulator_diffing_adjacent_element_node_t) lhs, AUGMENTED_DEQUE_EXAMPLE_PREFIXING(projector_diffing_adjacent_element_node_t) const &rhs) const
{
    using namespace augmented_containers::detail::tagged_ptr_ns;
    auto iterator_projected_storage_rhs = decltype(AUGMENTED_DEQUE_EXAMPLE_PREFIXING(augmented_deque))::sequence_t<0>::iterator_projected_storage_t::from_projected_storage_address(&const_cast<AUGMENTED_DEQUE_EXAMPLE_PREFIXING(projector_diffing_adjacent_element_node_t) &>(rhs));
...
AUGMENTED_DEQUE_EXAMPLE_PREFIXING(accumulator_diffing_adjacent_element_node_t) AUGMENTED_DEQUE_EXAMPLE_PREFIXING(accumulating_diffing_adjacent_element_binary_functor_t)::operator()(AUGMENTED_DEQUE_EXAMPLE_PREFIXING(accumulator_diffing_adjacent_element_node_t) lhs, AUGMENTED_DEQUE_EXAMPLE_PREFIXING(accumulator_diffing_adjacent_element_node_t) rhs) const
{
    using namespace augmented_containers::detail::tagged_ptr_ns;
    if(lhs.front == nullptr && lhs.back == nullptr)
    {
...
void AUGMENTED_DEQUE_EXAMPLE_PREFIXING(iterator_element_set_index)(std::size_t iterator_element_index, std::size_t index){ AUGMENTED_DEQUE_EXAMPLE_PREFIXING(augmented_deque_iterators_element)[iterator_element_index] = AUGMENTED_DEQUE_EXAMPLE_PREFIXING(augmented_deque).sequence<0>().begin_element() + index; }
...
void AUGMENTED_DEQUE_EXAMPLE_PREFIXING(iterator_element_advance)(std::size_t iterator_index, std::ptrdiff_t offset) { AUGMENTED_DEQUE_EXAMPLE_PREFIXING(augmented_deque_iterators_element)[iterator_index] += offset; }
...
void AUGMENTED_DEQUE_EXAMPLE_PREFIXING(pop_back)()
{
    if(!AUGMENTED_DEQUE_EXAMPLE_PREFIXING(augmented_deque).empty())
    {
        ...
        std::transform(AUGMENTED_DEQUE_EXAMPLE_PREFIXING(augmented_deque_iterators_element).begin(), AUGMENTED_DEQUE_EXAMPLE_PREFIXING(augmented_deque_iterators_element).end(), std::back_inserter(iterator_element_is_back_list), [](auto iterator_element)
            { return iterator_element == --unmove(AUGMENTED_DEQUE_EXAMPLE_PREFIXING(augmented_deque).sequence<0>().end_element()); });
#else
                ...
                .projected_and_accumulated_storage_to_string_converters =
                    ...
                    std::make_pair(
                        [](auto parameters) -> html_label_t
                        {
                            ...
                            auto iterator_projected_storage = decltype(AUGMENTED_DEQUE_EXAMPLE_PREFIXING(augmented_deque))::sequence_t<0>::iterator_projected_storage_t::from_projected_storage_address(&const_cast<AUGMENTED_DEQUE_EXAMPLE_PREFIXING(projector_diffing_adjacent_element_node_t) &>(parameters.datum));
                            if(iterator_projected_storage->prev != nullptr)
                            {
                                ...
                                {
                                    auto iterator_projected_storage_prev = decltype(AUGMENTED_DEQUE_EXAMPLE_PREFIXING(augmented_deque))::sequence_t<0>::iterator_projected_storage_t::from_projected_storage_address(iterator_projected_storage->prev);
                                    parameters.converter_generated_statements_back_inserter++ = edge_statement_t{{{
                                                                                                                     node_id_t{u8"list_node_" + object_pointer_to_string(iterator_projected_storage.current_list_node), u8"projected_storage.prev", c},
                            ...
                            if(iterator_projected_storage->next != nullptr)
                            {
                                ...
                                {
                                    auto iterator_projected_storage_next = decltype(AUGMENTED_DEQUE_EXAMPLE_PREFIXING(augmented_deque))::sequence_t<0>::iterator_projected_storage_t::from_projected_storage_address(iterator_projected_storage->next);
                                    parameters.converter_generated_statements_back_inserter++ = edge_statement_t{{{
                                                                                                                     node_id_t{u8"list_node_" + object_pointer_to_string(iterator_projected_storage.current_list_node), u8"projected_storage.next", c},
                        ...
                        [](auto parameters) -> html_label_t
                        {
                            ...
                            case accumulated_storage_node_type_t::digit_node_end_accumulated_storage:
                            {
                                ...
                                {
                                    auto iterator_projected_storage_front = decltype(AUGMENTED_DEQUE_EXAMPLE_PREFIXING(augmented_deque))::sequence_t<0>::iterator_projected_storage_t::from_projected_storage_address(tagged_ptr_bit0_unsetted(parameters.datum.front));
                                    parameters.converter_generated_statements_back_inserter++ = edge_statement_t{{{
                                ...
                                {
                                    auto iterator_projected_storage_back = decltype(AUGMENTED_DEQUE_EXAMPLE_PREFIXING(augmented_deque))::sequence_t<0>::iterator_projected_storage_t::from_projected_storage_address(tagged_ptr_bit0_unsetted(parameters.datum.back));
                                    parameters.converter_generated_statements_back_inserter++ = edge_statement_t{{{
                            ...
                            {
                                auto digit_node = decltype(AUGMENTED_DEQUE_EXAMPLE_PREFIXING(augmented_deque))::sequence_t<0>::digit_node_t::from_accumulated_storage_address(&const_cast<AUGMENTED_DEQUE_EXAMPLE_PREFIXING(accumulator_diffing_adjacent_element_node_t) &>(parameters.datum));
                                if(digit_node->p_accumulated_storage()->front != nullptr)
                                {
                                    auto iterator_projected_storage_front = decltype(AUGMENTED_DEQUE_EXAMPLE_PREFIXING(augmented_deque))::sequence_t<0>::iterator_projected_storage_t::from_projected_storage_address(tagged_ptr_bit0_unsetted(digit_node->p_accumulated_storage()->front));
                                    parameters.converter_generated_statements_back_inserter++ = edge_statement_t{{{
                                ...
                                {
                                    auto iterator_projected_storage_back = decltype(AUGMENTED_DEQUE_EXAMPLE_PREFIXING(augmented_deque))::sequence_t<0>::iterator_projected_storage_t::from_projected_storage_address(tagged_ptr_bit0_unsetted(digit_node->p_accumulated_storage()->back));
                                    parameters.converter_generated_statements_back_inserter++ = edge_statement_t{{{
                            ...
                            {
                                auto tree_node = decltype(AUGMENTED_DEQUE_EXAMPLE_PREFIXING(augmented_deque))::sequence_t<0>::tree_node_t::from_accumulated_storage_address(&const_cast<AUGMENTED_DEQUE_EXAMPLE_PREFIXING(accumulator_diffing_adjacent_element_node_t) &>(parameters.datum));
                                if(tree_node->p_accumulated_storage()->front != nullptr)
                                {
                                    auto iterator_projected_storage_front = decltype(AUGMENTED_DEQUE_EXAMPLE_PREFIXING(augmented_deque))::sequence_t<0>::iterator_projected_storage_t::from_projected_storage_address(tagged_ptr_bit0_unsetted(tree_node->p_accumulated_storage()->front));
                                    parameters.converter_generated_statements_back_inserter++ = edge_statement_t{{{
                                ...
                                {
                                    auto iterator_projected_storage_back = decltype(AUGMENTED_DEQUE_EXAMPLE_PREFIXING(augmented_deque))::sequence_t<0>::iterator_projected_storage_t::from_projected_storage_address(tagged_ptr_bit0_unsetted(tree_node->p_accumulated_storage()->back));
                                    parameters.converter_generated_statements_back_inserter++ = edge_statement_t{{{
